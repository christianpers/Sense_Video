...
void main() {
    ...
    vec2 pos = vec2(st * (mouse.x / 10.0) * 30.0);
    ...
    if (intSt.x == 2.0) {
      fractSt = rotate2D(fractSt, 1.0 * PI);
    }

    origSt -= vec2(0.5);
    origSt = scale( vec2(n)) * origSt;
    origSt += vec2(0.5);

    textureColor = texture2D( uTexture, origSt ).rgb;
    

    // fractSt -= vec2(0.5);
    // fractSt = scale( vec2(n * max(.9, abs(sin(u_time * PI)) * 2.0 ))) * fractSt;
    // fractSt += vec2(0.5);
